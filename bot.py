...
def get_plan_label(key: str) -> str:
    """اسم پلن رو برمی‌گردونه — اگه حجم از پنل ادمین تغییر کرده، اسم هم آپدیت میشه"""
    # پلن‌های اصلی: اسم پایه + حجم جدید (اگه override شده)
    if key in config.PLANS:
        base_name = config.PLANS[key]["name"]
        # بخش اول اسم (مثلاً "اشتراک") رو نگه می‌داریم، حجم رو از دیتابیس می‌خونیم
        size_override = db.get_plan_size(key)
        if size_override:
            # اسم پایه مثل "اشتراک ۱ گیگ" → "اشتراک" + حجم جدید
            parts = base_name.split(" ", 1)
            prefix = parts[0]  # "اشتراک"
            return f"{prefix} {size_override}"
        return base_name
    # بقیه پلن‌ها (تست، رفرال، بسته‌ای)
    return _PLAN_LABELS_DEFAULT.get(key, key)

# سازگاری با کدهای قدیمی که از PLAN_LABELS استفاده می‌کنن
class _PlanLabelsProxy(dict):
    def get(self, key, default=None):
        return get_plan_label(key) if key in _PLAN_LABELS_DEFAULT or key in config.PLANS else default
    def __getitem__(self, key):
        return get_plan_label(key)

PLAN_LABELS = _PlanLabelsProxy(_PLAN_LABELS_DEFAULT)
...
async def on_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    elif d.startswith("pay_disc_wallet_"):
        # pay_disc_wallet_{code}_{ptype}_{plan_key}
        rest = d[16:]  # بعد از "pay_disc_wallet_"
        parts = rest.split("_", 1)
        code = parts[0]
        key = parts[1] if len(parts) > 1 else ""
        await do_disc_wallet_payment(q, uid, code, key, context)

    elif d.startswith("pay_disc_card_"):
        # pay_disc_card_{code}_{ptype}_{plan_key}
        rest = d[14:]
        parts = rest.split("_", 1)
        code = parts[0]
        key = parts[1] if len(parts) > 1 else ""
        await do_disc_card_payment(q, uid, code, key, context)
...
async def do_disc_wallet_payment(q, uid, code, key, context):
    """پرداخت با موجودی + کد تخفیف"""
    dc = db.get_discount_code(code)
    if not dc:
        await q.edit_message_text("❌ کد تخفیف معتبر نیست یا منقضی شده."); return

    parts = key.split("_", 1); ptype = parts[0]; plan_key = parts[1] if len(parts) > 1 else ""
    if ptype == "bundle":
        plan = config.BUNDLE_PLANS.get(plan_key)
    else:
        plan = (config.PLANS if ptype == "sub" else config.TEST_PLANS).get(plan_key)
    if not plan:
        await q.edit_message_text("❌ پلن یافت نشد."); return

    base = vip_price(plan_key, uid)
    p = int(base * (100 - dc["percent"]) / 100)
    u = db.get_user(uid)
    if not u or u["balance"] < p:
        await q.edit_message_text(
            f"❌ موجودی کافی نیست.\n💰 موجودی شما: {fmt(u['balance'] if u else 0)}\n💵 مبلغ نیاز: {fmt(p)}",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("❌ بستن", callback_data="cancel")]]))
        return
    ...
    cfg = db.assign_config(plan_key, uid)
    db.confirm_payment(pay_id, cfg or "")
    db.create_subscription(uid, pay_id, plan_key, plan["name"], plan.get("size", 0), p, cfg or "")
    u = db.get_user(uid)
    if cfg:
        try:
            await context.bot.send_message(uid,
                f"✅ *خرید موفق — xservpn!*\n\n🔖 فاکتور: `{inv}`\n📦 {plan['name']}\n"
                f"🎫 کد تخفیف: `{code}` ({dc['percent']}% تخفیف)\n💵 مبلغ پرداختی: {fmt(p)}\n"
                f"💰 موجودی باقی‌مانده: {fmt(u['balance'] if u else 0)}\n\n🔑 کانفیگ شما:",
                parse_mode="Markdown")
            await context.bot.send_message(uid, cfg)
        except Exception: pass
        for aid in all_admins():
            try:
                await context.bot.send_message(aid,
                    f"🛍 *خرید با موجودی+تخفیف — کانفیگ ارسال شد*\n\n{uinfo(u)}\n\n"
                    f"🔖 فاکتور: `{inv}`\n📦 {plan['name']}\n🎫 کد: `{code}` ({dc['percent']}%)\n💵 {fmt(p)}\n✅ کانفیگ ارسال شد",
                    parse_mode="Markdown")
            except Exception: pass
        await q.edit_message_text(
        ...
        for aid in all_admins():
            try:
                await context.bot.send_message(aid,
                    f"⚠️ *خرید با موجودی+تخفیف — نیاز به کانفیگ*\n\n{uinfo(u)}\n\n"
                    f"🔖 فاکتور: `{inv}`\n📦 {plan['name']}\n🎫 کد: `{code}` ({dc['percent']}%)\n💵 {fmt(p)}\n⚠️ کانفیگ موجود نبود",
                    parse_mode="Markdown",
                    reply_markup=InlineKeyboardMarkup([
                        [InlineKeyboardButton("📤 ارسال کانفیگ دستی", callback_data=f"asc_{pay_id}")],
...
async def do_disc_card_payment(q, uid, code, key, context):
    """پرداخت با کارت + کد تخفیف"""
    dc = db.get_discount_code(code)
    if not dc:
        await q.edit_message_text("❌ کد تخفیف معتبر نیست یا منقضی شده."); return
    ...
    p = int(base * (100 - dc["percent"]) / 100)
    pay_id, inv = db.create_payment(uid, p, ptype, plan_key, plan["name"], pay_method="card")
    ss(uid, {"w": "upload_receipt", "pay_id": pay_id, "disc_code": code, "disc_pct": dc["percent"]})
    await q.edit_message_text(
        f"💳 *پرداخت با کارت — xservpn*\n\n"
        f"📦 پلن: {plan['name']}\n"
        f"🎫 کد تخفیف: `{code}` ({dc['percent']}% تخفیف)\n"
        f"💵 مبلغ نهایی: *{fmt(p)}*\n\n"
        f"💳 شماره کارت: `{card()}`\n"
        f"👤 صاحب کارت: {cardholder()}\n\n"
        f"🔖 شناسه فاکتور: `{inv}`\n\n"
        f"پس از واریز، تصویر رسید را ارسال کنید:",
        parse_mode="Markdown")
